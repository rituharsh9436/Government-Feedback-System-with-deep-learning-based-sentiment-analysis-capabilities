...
function App() {
  ...
  const signup = (event) => run('signup', async () => {
    ...
    setMode('login'); setSignupRole('public'); notify(data.role === 'govt' ? 'Request submitted for admin approval.' : 'Account created. Please sign in.');
  });
  ...
  if (!token || !currentUser) return <main className="auth-shell"><section className="brand"><span>PUBLIC VOICE</span><h1>Smart Government Feedback</h1><p>Policies made clearer through public participation.</p></section><section className="auth-card"><h2>{mode === 'login' ? 'Welcome back' : 'Create an account'}</h2>{message && <p className={`notice ${message.type}`}>{message.text}</p>}{mode === 'login' ? <form onSubmit={login}><label>Email<input required name="email" type="email" autoComplete="email" /></label><label>Password<input required name="password" type="password" autoComplete="current-password" /></label><button disabled={busy.login}>{busy.login ? 'Signing in…' : 'Sign in'}</button><button className="link" type="button" onClick={() => setMode('signup')}>New here? Register</button></form> : <form onSubmit={signup}><label>Full name<input required name="full_name" minLength="2" /></label><label>Email<input required name="email" type="email" autoComplete="email" /></label><label>Password<input required name="password" type="password" minLength="8" autoComplete="new-password" /></label><label>Account type<select name="role" value={signupRole} onChange={(event) => setSignupRole(event.target.value)}><option value="public">Public</option><option value="govt">Government (requires approval)</option></select></label>{signupRole === 'public' && <label>Aadhaar number<input required name="aadhaar_number" pattern="[0-9]{12}" title="12 digits" /></label>}<label>Contact number<input required name="contact_number" pattern="\+?[1-9][0-9]{7,14}" /></label>{signupRole === 'govt' && <><label>Department name<input required name="department_name" minLength="2" maxLength="150" /></label><label>Department ID<input required name="department_id" minLength="2" maxLength="50" /></label></>}<button disabled={busy.signup}>{busy.signup ? 'Creating…' : 'Create account'}</button><button className="link" type="button" onClick={() => setMode('login')}>Back to sign in</button></form>}</section></main>;

...
function App() {
  ...
  const savePolicy = (form) => run('policy', async () => {
    const path = editingPolicy ? `/posts/${editingPolicy._id}` : '/posts/';
    await authenticatedRequest(path, { method: editingPolicy ? 'PUT' : 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) });
    setEditingPolicy(null); notify(editingPolicy ? 'Policy updated.' : 'Policy published.'); reloadPolicies();
  });
  ...
  return <main><header><div><span className="eyebrow">PUBLIC VOICE</span><h1>Policy Forum</h1></div><div className="user">Signed in as <b>{currentUser.full_name}</b><span className="role">{currentUser.role}</span><button className="outline" onClick={() => logout()}>Sign out</button></div></header><div className="content">{message && <p className={`notice ${message.type}`}>{message.text}</p>}
    {isGovernment && <section className="panel"><div className="row"><h2>{editingPolicy ? 'Edit policy' : 'Publish a policy'}</h2><button className="outline" onClick={getOverallAnalysis} disabled={busy['overall-analysis']}>{busy['overall-analysis'] ? 'Loading…' : 'Overall feedback analysis'}</button></div>{overallAnalysis && <p className="analysis">Policies: {overallAnalysis.policy_count} · Replies: {overallAnalysis.comment_count} · Sentiment: {overallAnalysis.analysis_status}</p>}<PolicyForm initialPolicy={editingPolicy} onSave={savePolicy} saving={busy.policy} onCancel={editingPolicy ? () => setEditingPolicy(null) : null} /></section>}

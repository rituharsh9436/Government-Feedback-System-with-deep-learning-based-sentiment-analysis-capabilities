...
2. Create the ml-service `.env` file from the example:
   ```bash
   cp ml-service/.env.example ml-service/.env
   ```
   Edit `ml-service/.env` and set:
   - `MODEL_ID`: Your fine-tuned Hugging Face MuRIL model ID (e.g., `username/my-muril-sentiment`).
   - `MODEL_REVISION`: The revision/branch of the model (e.g., `main`).
   - `HF_TOKEN`: (Optional) Your Hugging Face token if the model is private.

3. Create the frontend `.env` file from the example:
   ```bash
...
Original work by the team: Harsh Bhardwaj, Chanchal Sharma, Harshita Gupta